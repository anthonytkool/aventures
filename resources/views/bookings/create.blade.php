  ...
  <form action="{{ route('bookings.store') }}" method="POST">
    ...
    <div class="row">
      ...
      <div class="col-md-6 mb-3">
        ...
        <select name="tour_departure_id" class="form-select">
          <option value="">-- Select a date --</option>
          @foreach($tour->departures as $departure)
            <option value="{{ $departure->id }}">
              {{ \Carbon\Carbon::parse($departure->start_date)->format('M d, Y') }}
              - {{ \Carbon\Carbon::parse($departure->end_date)->format('M d, Y') }}
